...
<enhancement>
The new behavior adds a movable placement marker when hovering the route, with cursor tracking and click-to-commit target placement.
</enhancement>

<decisions>
1. Hover placement is transient preview state, separate from committed draft markers.
2. The preview comes from a draft-only segment hover detector that projects a point between draft markers.
3. The preview marker is visual-only; pointer-up on the map performs the insert-into-segment transition and turns the preview point into a committed target marker.
...
Check how a draft-only segment hover detector can project a point between draft markers and how click-up inserts that projected point into the draft.
...
4. As an enhancement, a movable placement marker appears at the cursor, using the circle style and numbered marker size.
5. The cursor changes to a pointing finger while the pointer remains over the route.
6. As the cursor moves, the placement marker tracks it.
7. User clicks the placement marker, which inserts it into the route segment and changes it to a target marker.
...
2. Add a draft-only segment hover detector that projects the nearest point on a draft route segment and reports enough state to render a cursor-tracking placement marker.
...
7. When the map receives pointer-up while a placement preview is active, insert the preview point into the current draft segment and render it as a target marker; the preview marker itself does not commit.
...
**Error Handling:**
12. Clear hover placement state on pointer exit, pointer cancel, route commit, and when route drafting ends.
13. If the pointer is touch-only or not actually over a draft segment, render the normal map without side effects.
14. Hover state changes must not mutate route geometry, numbering, selection state, persistence, or export behavior until the click commit occurs.

**Edge Cases:**
15. The hover placement marker should track smoothly during mouse movement and not lag behind the cursor.
16. Rapid hover enter/exit during panning must not leave a stale hover placement behind.
17. Narrow-screen route sheet layout and existing route save behavior must remain unchanged.
</requirements>
...
4. Add a failing robot journey for create draft -> hover segment -> move cursor -> click commit -> verify inserted target state with stable keys.
...
<done_when>
Hovering a draft route segment shows a movable placement marker that matches the circle style, uses numbered-marker size, and tracks the cursor.
The cursor becomes a pointing finger while hovering the segment.
Clicking the placement marker inserts it into the draft segment and changes it to a target marker.
