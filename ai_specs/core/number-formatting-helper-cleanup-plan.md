...
- [ ] `lib/screens/settings_screen.dart` - format reviewed counts in peak refresh, route-graph refresh, peak export, peak-list export, track reset, track recalc, Tassy Full result copy
- [ ] `lib/providers/map_provider.dart` - format import and recalc status summaries with shared helpers
- [ ] `lib/services/peak_refresh_service.dart` - format skipped warning count
- [ ] `lib/services/peak_repository.dart` - format malformed `PeakList` warning count
- [ ] `lib/widgets/gpx_import_dialog.dart` - format added/unchanged/unsupported/error counts
- [ ] `lib/widgets/peak_list_import_dialog.dart` - format imported/skipped/warning counts
- [ ] `lib/screens/map_screen_panels.dart` - format `tracks available` count
- [ ] `test/harness/test_map_notifier.dart` - update canned status strings to formatted text
- [ ] TDD: `test/widget/peak_refresh_settings_test.dart` - peak refresh dialog/status keeps formatted count text and warning text
- [ ] TDD: `test/widget/route_graph_refresh_settings_test.dart` - route-graph element counts use shared formatting
- [ ] TDD: `test/widget/peak_csv_export_settings_test.dart` - peak export success status uses formatted count text
- [ ] TDD: `test/widget/peak_list_csv_export_settings_test.dart` - export summary uses formatted exported/skipped/warning counts
- [ ] TDD: `test/widget/gpx_import_dialog_test.dart` - import-result dialog uses formatted counts
- [ ] TDD: `test/widget/gpx_tracks_summary_test.dart` - settings summary mirrors formatted provider status strings
- [ ] TDD: `test/widget/gpx_tracks_shell_test.dart` - snackbar/detail mirrors formatted provider status strings where asserted
- [ ] TDD: `test/widget/tassy_full_peak_list_settings_test.dart` - Tassy Full result dialog uses formatted added/updated counts
- [ ] Robot journey tests + selectors/seams: `test/robot/peaks/peak_refresh_journey_test.dart`, `test/robot/peaks/peak_list_export_journey_test.dart`, `test/robot/gpx_tracks/gpx_tracks_robot.dart` - update assertions only; reuse existing keys unless a missing stable selector blocks coverage
