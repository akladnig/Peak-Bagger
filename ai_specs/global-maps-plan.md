...
- [x] `./lib/screens/map_screen.dart` - own drawer-region snapshot as route-local state; pass snapshot into drawer builder
- [x] `./lib/widgets/map_basemaps_drawer.dart` - render manifest-filtered basemaps for snapped region; show empty/unavailable state; fallback current selection to `Basemap.tracestrack` when needed
- [x] `./lib/screens/map_screen_layers.dart` - replace `mapTileUrl` switch with catalog lookup only
- [x] `./lib/widgets/map_action_rail.dart` - keep FAB open behavior aligned with new snapshot seam if plumbing changes
- [x] TDD: drawer snapshot stays stable while open; cursor-first then center fallback; hidden basemaps excluded; unavailable active basemap falls back to tracestrack
- [x] Robot selectors: `Key('show-basemaps-fab')`, `Key('basemaps-drawer')`, `Key('basemap-option-${basemap.name}')`-style option keys
- [x] Verify: `flutter analyze` && `flutter test test/widget/map_basemaps_drawer_test.dart test/widget/map_screen_rebuild_test.dart`

### Phase 3: Cache sync + journey proof

- **Goal**: keep cache warmup/store init aligned; prove critical user journey
- [x] `./lib/services/tile_cache_service.dart` - derive store init/warmup iteration from manifest-backed enum ordering
- [x] `./test/unit/tile_cache_service_test.dart` - update ordering/assertions for expanded enum/catalog
- [x] `./test/robot/map/basemap_selection_journey_test.dart` - critical journey: open drawer, region-resolved list, select basemap, confirm map updates
- [x] TDD: warmup/store order matches enum order; selected basemap selection path still updates map; stale/hidden basemap selection falls back cleanly
- [x] Verify: `flutter analyze` && `flutter test test/unit/tile_cache_service_test.dart test/robot/map/basemap_selection_journey_test.dart`
