...
The task source is `./ai_specs/list-backup.md`. The generated implementation should save code changes in the existing app structure and should not create a new app or command-line tool.
...
<discovery>
1. `file_picker` is pinned to `^10.3.10`; the installed API includes `FilePicker.platform.getDirectoryPath({String? dialogTitle, bool lockParentWindow = false, String? initialDirectory})` and documents `initialDirectory` support for Linux, macOS, and Windows.
2. `PeakListFilePicker.resolveImportRoot()` already implements the required import-root convention: `~/Documents/Bushwalking` when present, otherwise home, otherwise `Directory.current.path`.
3. Settings confirmation/result dialogs are centralized in `./lib/widgets/dialog_helpers.dart`; `showDangerConfirmDialog` supports keyed confirm/cancel buttons, and `showSingleActionDialog` supports a keyed close button.
4. Settings widget tests already exist under `./test/widget/*settings*_test.dart`, including `./test/widget/peak_refresh_settings_test.dart`, and use `ProviderScope`/`UncontrolledProviderScope` overrides plus stable keys.
5. Existing robot-style journeys live under `./test/robot/gpx_tracks/` and navigate to Settings through `router.go('/settings')`; a Settings export robot should follow that key-first style rather than relying on text selectors.
6. `PeakListRepository.getAllPeakLists()`, `PeakRepository.getAllPeaks()`, and `PeakRepository.findByOsmId()` expose the needed read paths; both repositories already have in-memory test storage implementations.
7. There is no existing shared filesystem/log-writing seam for this feature. Add a minimal export-specific filesystem seam instead of touching real user directories in tests.
</discovery>
...
8. App prepares an export plan by snapshotting repository data, generating target paths, row counts, and warnings without writing CSV files or appending warning logs.
9. If generated output files already exist, app shows an overwrite confirmation before writing anything.
10. App commits the requested CSV output.
11. App shows a success dialog with exported file count, exported row count, warning count, and the `backup.log` path when warnings were logged.
...
- Logging to `backup.log` fails: CSV export should still succeed, and the result dialog should mention that warning log writing failed.
- Malformed peak-list payload: skip that peak list, append a warning to `backup.log`, and include it in the success-dialog warning count.
- Peak-list item references a missing `Peak.osmId`: skip that row, append a warning to `backup.log`, and include it in the success-dialog warning count.
</user_flows>
...
3. `Export Peak Lists` must export one CSV file per stored `PeakList` entity.
...
18. The initial output-folder root must reuse the existing import-root convention from `PeakListFilePicker.resolveImportRoot()`: `~/Documents/Bushwalking` when it exists, otherwise the user home directory, otherwise the current directory.
19. Append warning log entries to `backup.log` in the resolved default root, not necessarily the selected output folder.
20. Warning log entries must include timestamp, export type, affected list name or target filename when available, and the warning message.
21. Successful exports with skipped rows/lists must still show a success dialog, not a failure dialog.
22. The success dialog must report exported file count, exported row count, warning count, and the absolute `backup.log` path when warnings were logged.
22a. If CSV export succeeds but appending `backup.log` fails, the success dialog must show `Could not update backup.log.`.
23. Use stable keys for the new Settings controls, dialogs, and status text. The existing dialog helpers only key action buttons, so do not require title/content keys unless the helper is explicitly extended.
...
27. If a selected output directory does not exist or is not writable, show a failure dialog and do not create partial CSV output.
...
29. If appending to `backup.log` fails after CSV output succeeds, keep the CSV files and show the export as successful with an additional warning that log writing failed.
30. If a peak-list payload cannot be decoded with `decodePeakListItems`, skip that list entirely and log/report a warning.
...
37. The export service must snapshot all required `Peak` and `PeakList` data before async filesystem writes so later in-app edits or refreshes cannot change the export contents mid-commit.
...
39. The export service must be testable with fake repositories, fake filesystem writes, and fake clock. The picker/root resolver seam may be tested separately or through a small UI runner/provider; the core CSV generation must not require a real platform file picker.
...
- These CSVs are not required to be import-compatible in this task. A later importer update will accept the exported `gridZoneDesignator` or `mgrs100kId` peak-list fields.
- Do not mutate ObjectBox data during export.

Filesystem boundaries:
- The selected output folder receives CSV files only.
- The resolved default root receives `backup.log` warning entries only.
...
<implementation>
1. Add a small export service, for example `./lib/services/data_export_service.dart`, responsible for transforming repository data into CSV rows, resolving warnings, creating deterministic filenames, detecting overwrite targets, and coordinating writes through an injected filesystem seam.
2. Use a prepare/commit service API. The prepare step takes the selected output directory, snapshots repository data, returns target paths, row counts, warnings, and overwrite conflicts, and performs no writes or log appends. The commit step takes an approved plan and writes CSV files/log entries.
3. Add a new file picker seam, for example `./lib/services/data_export_file_picker.dart`, with `pickOutputDirectory()` and `resolveDefaultExportRoot()`. The platform implementation should call `FilePicker.platform.getDirectoryPath(initialDirectory: await resolveDefaultExportRoot())`.
4. Add a filesystem seam for existence checks, writability checks, temporary-file writes, final replacement/rename, cleanup, and appending `backup.log` so service tests do not touch the real filesystem.
5. Add Riverpod providers for the export service and file picker/filesystem seams, likely in a new focused provider file such as `./lib/providers/data_export_provider.dart` to avoid overloading import-specific providers.
6. Update `./lib/screens/settings_screen.dart` to render the `List Exports` section as the last persistent Settings section in the `ListView`; transient status text may remain after it if that matches the current Settings pattern.
7. Reuse `showDangerConfirmDialog` for initial export confirmation and overwrite confirmation.
8. Reuse `showSingleActionDialog` for success and failure results.
9. Add stable keys, including at minimum: `data-export-section`, `export-peak-lists-tile`, `export-peaks-tile`, `export-peak-lists-confirm`, `export-peak-lists-cancel`, `export-peaks-confirm`, `export-peaks-cancel`, `export-overwrite-confirm`, `export-overwrite-cancel`, `data-export-status`, `data-export-result-close`, and `data-export-error-close`.
10. Keep UI state local and minimal unless existing Settings patterns require a provider; `_isExportingPeakLists`, `_isExportingPeaks`, and a separate List Exports status string rendered with `data-export-status` are acceptable.
11. Use temporary files in the selected output directory where practical: generate all CSV content first, write temporary CSV files, then replace/create final filenames only after all temporary writes have succeeded. Final replacement is best-effort across multiple files; if it fails, clean up remaining temp files and report that the export may be partially written.
12. Prefer small data/result classes such as `DataExportResult`, `DataExportWarning`, `DataExportTarget`, and an overwrite/conflict result only if they make service and UI testing clearer.
13. Avoid broad backup terminology in UI labels beyond the requested `backup.log`; the user-facing feature name is `List Exports`.
14. Do not add new packages unless required after discovery; `csv`, `file_picker`, and `path` already exist in `./pubspec.yaml`.
</implementation>
...
11. Test prepare/commit behavior: prepare reports overwrite conflicts and warnings without writing CSV files or appending `backup.log`; commit writes only after overwrite is approved.
12. Test warning log entries are appended to `backup.log` in the resolved default root with timestamp, export type, context, and message.
...
20a. Test the List Exports status uses `data-export-status` and does not reuse `peak-refresh-status`.
...
23. Cover the warning path: fake export reports skipped malformed/missing data, success dialog shows warnings, and the visible result includes the `backup.log` path or log-write failure message.
...
Manual verification:
28. Run `flutter test`.
29. Manually run the app on macOS if feasible, export peaks and peak lists to a temporary folder, inspect the generated CSV files, and confirm `backup.log` appears in `~/Documents/Bushwalking` when warnings occur.
</validation>

<done_when>
1. `./lib/screens/settings_screen.dart` shows a `List Exports` section with `Export Peak Lists` and `Export Peaks` actions.
2. `Export Peak Lists` writes one CSV per peak list to a user-selected folder with the required headers, filename format, row mapping, and warning behavior.
...
5. Malformed peak-list payloads and missing peak references are skipped, reported in the success dialog, and appended to `backup.log` in the resolved default root with the log path shown to the user.
6. Cancellation and failure paths do not create unexpected output files.
7. Automated coverage exists for service behavior, Settings UI behavior, and robot-driven critical journeys.
