...
F-5a. When a GPX file is moved, rename it to a canonical filename unless the source filename is already in canonical form `all-lower-case-name-with-dashes-in-between-words_(dd-mm-yyy).gpx`, in which case keep the original filename; otherwise lowercase, replace whitespace with `-`, collapse repeated dashes, replace `.`/` ,`/`&` with `-`, then append `_(dd-mm-yyyy)` before the `.gpx` extension using a filename date/datetime in parentheses when present, otherwise using the derived trackDate
