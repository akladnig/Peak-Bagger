...
- Tasmania bounds: latitude -39 to -44, longitude 143 to 148
...
8. If the track is Tasmanian and not already in the Tasmania folder, move it into the Tasmania folder using collision rules
9. If the track is non-Tasmanian and currently in the Tasmania folder, move it to the parent Tracks folder using collision rules
10. Parse the GPX into segmented geometry
11. If an existing row has the same contentHash, do not create another persisted row; use the canonical persisted trackName/trackDate chosen for that duplicate group rather than whichever later filename was scanned, attempt on-disk organization for the duplicate, and count it as unchangedCount only if organization succeeds without a destination-path collision
12. Persist and render only Tasmanian tracks; non-Tasmanian tracks are organized on disk only and are not inserted into ObjectBox or shown on the map
13. If an existing row has the same logical track match (trackName + trackDate) but different contentHash, replace the prior row in the database only when both the incoming track and the persisted row have startDateTime != null, which marks trackDate as metadata-derived rather than file-modification-derived
14. This replacement rule is an accepted tradeoff and may destructively replace distinct tracks that share the same trackName and normalized trackDate
15. Logical-match replacement first moves the new file using its existing filename, then replaces the database row
16. For logical-match replacement only, overwrite is allowed only when the current move target path already exists and the existing destination file resolves to the same logical match (trackName + trackDate)
17. This overwrite rule is also an accepted destructive tradeoff and may overwrite the wrong file when multiple files share the same logical match
18. The feature does not track or delete old organized files on disk
19. Otherwise insert a new track row
20. During manual rescan, already-visible tracks remain visible until refreshed data is ready
21. User taps the show tracks FAB to display or hide imported tracks
22. Tracks render on the map with color #a726bc
...
- First track point unreadable: skip file, continue
...
F-5. Auto-organize tracks: move Tasmanian tracks to Tasmania folder and non-Tasmanian tracks to the parent folder
F-6. Persist and render only Tasmanian tracks; non-Tasmanian tracks are organized on disk only and are not inserted into ObjectBox or shown on the map
F-7. Use contentHash to detect unchanged persisted Tasmanian tracks during rescans; non-Tasmanian files are not stored in ObjectBox and are reclassified/reorganized on each routine rescan instead
...
F-10. Add an import FAB with Icons.input, tooltip text "Import track", and a matching semantics label
...
F-29. nonTasmanianCount counts scanned non-Tasmanian GPX files that are organized on disk only and not inserted into ObjectBox
F-30. errorSkippedCount counts GPX files not imported into ObjectBox because they were invalid, unreadable, blocked by collisions, blocked by overwrite verification, or left at source path for manual review after losing a same-operation logical-match conflict
F-31. Per-file skip reasons that represent an error or manual-review condition must be recorded in import.log with the filename and reason
...
**Error Handling:**
ERR-1. Missing folders: create ~/Documents/Bushwalking/Tracks and the Tasmania subfolder
ERR-2. Invalid GPX: skip the file, continue processing, and record the filename and reason in import.log
ERR-3. First track point unreadable: skip the file, continue processing, and record the filename and reason in import.log
...
ERR-10. Failed track imports and manual-review skips must append a timestamp, filename, and reason to ~/Documents/Bushwalking/Tracks/Tasmania/import.log
...
EC-8. During logical-match replacement, move the new file using its existing filename and allow overwrite only when the current move target path already exists and the existing destination file resolves to the same logical match
...
- File renaming is out of scope for this feature
...
- Logical-match replacement moves the new file using its existing filename and may overwrite only when the current move target path already exists and the existing destination file resolves to the same logical match; this is an accepted destructive tradeoff when multiple files share the same logical match
</boundaries>
...
- @lib/services/gpx_importer.dart
  ...
  - move files using collision rules
  - move files without renaming them during logical-match replacement
  ...
  - append a timestamp, filename, and skip/failure reason to ~/Documents/Bushwalking/Tracks/Tasmania/import.log
...
- @lib/screens/settings_screen.dart
  ...
  - place the inline recovery warning and helper copy directly with the Reset Track Data action, including copy that explains Reset Track Data rebuilds from both Tracks and Tracks/Tasmania because routine manual rescans ignore the organized Tasmania folder
...
- Unit test: GPXImporter preserves original filenames when moving files
...
- Unit test: Failed track imports append a timestamp, filename, and reason to ~/Documents/Bushwalking/Tracks/Tasmania/import.log
...
- Unit test: logical-match replacement moves the new file using its existing filename before replacing the database row
...
- Unit test: only Tasmanian tracks are persisted while non-Tasmanian tracks are organized on disk only
...
5. Import logic scans the correct macOS folders for initial and later imports, persists and renders only Tasmanian tracks, treats files in Tracks/Tasmania as out of scope for routine rescans, organizes tracks correctly without renaming files, skips non-replacement destination path collisions with a manual-review error, rolls back failed replacement commits after a successful move, replaces matched prior rows when content changes only for metadata-date-eligible tracks, imports changed no-date tracks as additional rows, and leaves later same-operation logical-match conflicts at source path for manual review as errorSkippedCount
