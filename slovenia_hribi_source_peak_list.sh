...
target_path='tool/slovenia_hribi_source_peak_list.dart'

if [ -n "${PEAK_BAGGER_SLOVENIA_TOOL_BINARY:-}" ]; then
  exec "${PEAK_BAGGER_SLOVENIA_TOOL_BINARY}" "$@"
fi

binary_path="$script_dir/build/macos/Build/Products/Release/peak_bagger.app/Contents/MacOS/peak_bagger"
build_stamp_path="$script_dir/build/macos/Build/Products/Release/.slovenia_hribi_source_cli_build_stamp"
...
if needs_build; then
  (
    cd "$script_dir"
    flutter build macos --release -t "$target_path"
