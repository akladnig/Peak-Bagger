...
void main() {
  ...
  test(
    'creates dry-run preview, logs unresolved rows, and logs unmatched peaks',
    () async {
...
''',
      ...
      expect(
        logWrites['${tempDir.path}/logs/prominence-unresolved-csv.log'],
        contains('action=unresolved-csv-row'),
      );
      expect(
        logWrites['${tempDir.path}/logs/prominence-not-found-in-dataset.log'],
        contains('action=not-found-in-dataset'),
      );

      final previewRows = const CsvDecoder().convert(
...
''',
  ...
  test(
    'continues after a write failure and reports the failure count',
    () async {
      final repository = PeakRepository.test(
        InMemoryPeakStorage([
          Peak(
...
''',
      ...
      expect(result.report.unmatchedPeakCount, 1);
      expect(
        logWrites['${tempDir.path}/logs/prominence.log'],
        contains('action=write-failure'),
      );
      expect(
        logWrites['${tempDir.path}/logs/prominence-not-found-in-dataset.log'],
        contains('action=not-found-in-dataset'),
