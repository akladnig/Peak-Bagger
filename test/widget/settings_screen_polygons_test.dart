...
void main() {
  testWidgets('settings screen shows polygon toggle row', (tester) async {
    ...
    expect(find.byKey(const Key('show-polygons-tile')), findsOneWidget);
    expect(find.byKey(const Key('show-polygons-switch')), findsOneWidget);
    expect(find.text('Show Polygons'), findsOneWidget);
  });

  testWidgets('polygon toggle persists across rebuilds', (tester) async {
    SharedPreferences.resetStatic();
    SharedPreferences.setMockInitialValues({});
    ...
    await tester.tap(find.byKey(const Key('show-polygons-tile')));
