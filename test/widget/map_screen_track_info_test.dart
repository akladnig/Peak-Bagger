...
void main() {
  testWidgets('wide selected track renders panel and narrow width hides it', (
    tester,
  ) async {
    ...
    expect(tester.getSize(find.byType(MapScreen)).width, greaterThanOrEqualTo(1024));
    ...
    await _pumpRawMapScreen(tester, state, size: const Size(800, 900));

    expect(find.byKey(const Key('track-info-panel')), findsNothing);
    final narrowContainer = ProviderScope.containerOf(
      tester.element(find.byKey(const Key('map-interaction-region'))),
    );
    expect(narrowContainer.read(mapProvider).selectedTrackId, 10);
    expect(find.byKey(const Key('map-mgrs-readout')), findsOneWidget);
    expect(find.byKey(const Key('map-zoom-readout')), findsOneWidget);
  });

  testWidgets('close button clears selected track and hides panel', (tester) async {
    final state = MapState(
      center: const LatLng(-41.5, 146.5),
      zoom: 15,
      basemap: Basemap.tracestrack,
  ...
  testWidgets('panel shows None fallback and pinned close button stays accessible', (
    tester,
  ) async {
    final track = GpxTrack(
      gpxTrackId: 11,
  ...
  testWidgets('basemaps drawer coexists with selected track panel', (tester) async {
    final track = GpxTrack(
      gpxTrackId: 10,
      contentHash: 'hash-10',
      trackName: 'Ridge Walk',
