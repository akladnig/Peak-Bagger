...
void main() {
  test(
    'route draft hover state sets, clears, and resets on draft lifecycle',
    () async {
  ...
  test(
    'route draft preview inserts into the ordered chain and clears hover',
    () {
      ...
      expect(
        container.read(mapProvider).hoveredRouteDraftCommittedSegmentIndex,
        0,
      );
      expect(
        container.read(mapProvider).hoveredRouteDraftSegmentPoint,
        preview,
      );
      ...
      expect(state.routeDraftControlEndpoints.map((e) => e.point), [
        a,
        preview,
        b,
        c,
  ...
  test('route draft preview promotes both adjacent auto-route anchors', () {
    const a = LatLng(-41.5, 146.47);
    const b = LatLng(-41.49, 146.49);
    const c = LatLng(-41.51, 146.51);
    const d = LatLng(-41.5, 146.53);
    const preview = LatLng(-41.495, 146.5);
    ...
    final notifier = TestMapNotifier(
      MapState(
        ...
        routeDraftMarkers: const [a, d],
        routeDraftCommittedPoints: const [a, b, c, d],
        routeDraftProvisionalPoints: const [],
      ),
    );
    final container = ProviderContainer(
    ...
    expect(state.routeDraftCommittedPoints, [a, b, preview, c, d]);
