...
void main() {
  test('returns 400 for malformed tile path', () async {
    final handler = FvgTopoTileHandler(
      upstreamClient: _FakeUpstreamWmsClient(),
    );

    final response = await handler.handle(
      Request('GET', Uri.parse('https://example.com/fvg-topo/nope/1.png')),
    );
  ...
  test(
    'returns transparent tile when request does not intersect FVG',
    () async {
      final upstream = _FakeUpstreamWmsClient();
      final handler = FvgTopoTileHandler(upstreamClient: upstream);
      ...
      final response = await handler.handle(
        Request(
          'GET',
          Uri.parse(
            'https://example.com/fvg-topo/${tile.z}/${tile.x}/${tile.y}.png',
          ),
        ),
      );
  ...
  test('uses exact projected bbox for intersecting tile request', () async {
    final upstream = _FakeUpstreamWmsClient();
    final handler = FvgTopoTileHandler(upstreamClient: upstream);
    final tile = _tileForLatLng(latitude: 46.1, longitude: 13.2, zoom: 12);
    final expectedBounds = tileBoundsInWebMercator(tile);
    ...
    expect(upstream.lastLayerName, 'IRDAT:CRN25KColore');
  });

  test('switches upstream layer by zoom band', () async {
    final cases = <int, String>{
      1: 'IRDAT:CRN25KColore',
      12: 'IRDAT:CRN25KColore',
      13: 'IRDAT:CTRN5KColore',
      19: 'IRDAT:CTRN5KColore',
    };
  ...
  test(
    ...
    () async {
      ...
      final response = await handler.handle(
        Request(
          'GET',
          Uri.parse('https://example.com/fvg-topo/20/566523/372803.png'),
        ),
      );
  ...
  test('retries transient upstream timeout before succeeding', () async {
    ...
    final handler = FvgTopoTileHandler(upstreamClient: upstream);
    final tile = _tileForLatLng(latitude: 46.1, longitude: 13.2, zoom: 13);
